...
from app.dto.match.response.matchListResponse import MatchListItem, MatchListData, MatchListResponse
...
class MatchService:
    ...
    def accept_match(self, db: Session, body: MatchAcceptRequest) -> MatchAcceptResponse:
        req, club1, club2 = self.repository.get_request_with_clubs(db, int(body.requestId))
        if req is None or club1 is None or club2 is None:
            return MatchAcceptResponse(status=500, message="제안을 수락하는데 실패했습니다.", data=None)
        self.repository.create_match_from_request(db, req)
        self.repository.delete_request(db, req)
        db.commit()
        return MatchAcceptResponse(status=200, message="제안을 수락했습니다.", data=None)


    def reject_match(self, db: Session, body: MatchAcceptRequest) -> MatchRejectResponse:
        req = self.repository.get_request_by_id(db, int(body.requestId))
        if req is None:
            return MatchRejectResponse(status=500, message="제안을 거절하는데 실패했습니다.", data=None)
        self.repository.delete_request(db, req)
        db.commit()
        return MatchRejectResponse(status=200, message="제안을 거절했습니다.", data=None)
    ...
    def list_received_requests(self, db: Session, user_id: str) -> MatchListResponse:
        ...
        if not user or not club:
            return MatchListResponse(status=500, message="동아리 가져오기 실패", data=MatchListData(items=[]))
